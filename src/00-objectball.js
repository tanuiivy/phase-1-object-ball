// Create the game object structure
function gameObject() {
    return {
      home: {
        teamName: "Brooklyn Nets",
        colors: ["Black", "White"],
        players: {
          "Alan Anderson": { number: 0, shoe: 16, points: 22, rebounds: 12, assists: 12, steals: 3, blocks: 1, slamDunks: 1 },
          "Reggie Evans": { number: 30, shoe: 14, points: 12, rebounds: 12, assists: 12, steals: 12, blocks: 12, slamDunks: 7 },
          "Brook Lopez": { number: 11, shoe: 17, points: 17, rebounds: 19, assists: 10, steals: 3, blocks: 1, slamDunks: 15 },
          "Mason Plumlee": { number: 1, shoe: 19, points: 26, rebounds: 12, assists: 6, steals: 3, blocks: 8, slamDunks: 5 },
          "Jason Terry": { number: 31, shoe: 15, points: 19, rebounds: 2, assists: 2, steals: 4, blocks: 11, slamDunks: 1 },
        },
      },
      away: {
        teamName: "Charlotte Hornets",
        colors: ["Turquoise", "Purple"],
        players: {
          "Jeff Adrien": { number: 4, shoe: 18, points: 10, rebounds: 1, assists: 1, steals: 2, blocks: 7, slamDunks: 2 },
          "Bismak Biyombo": { number: 0, shoe: 16, points: 12, rebounds: 4, assists: 7, steals: 7, blocks: 15, slamDunks: 10 },
          "DeSagna Diop": { number: 2, shoe: 14, points: 24, rebounds: 12, assists: 12, steals: 4, blocks: 5, slamDunks: 5 },
          "Ben Gordon": { number: 8, shoe: 15, points: 33, rebounds: 3, assists: 2, steals: 1, blocks: 1, slamDunks: 0 },
          "Brendan Haywood": { number: 33, shoe: 15, points: 6, rebounds: 12, assists: 12, steals: 22, blocks: 5, slamDunks: 12 },
        },
      },
    };
  }
  
  // Example function: Returns points for a given player
  function numPointsScored(playerName) {
    const game = gameObject();
    for (let team in game) {
      const players = game[team].players;
      if (players[playerName]) {
        return players[playerName].points;
      }
    }
    return "Player not found!";
  }
  
  console.log(numPointsScored("Ben Gordon")); // Example test
  
  function mostPointsScored() {
    let game = gameObject();
    let highestPoints = 0;
    let playerWithMostPoints = "";
  
    for (let teamKey in game) {
      let team = game[teamKey];
      for (let playerName in team.players) {
        let player = team.players[playerName];
        if (player.points > highestPoints) {
          highestPoints = player.points;
          playerWithMostPoints = playerName;
        }
      }
    }
    return playerWithMostPoints;
  }
  
  console.log(mostPointsScored()); // Should log the player with the most points
  
  function winningTeam() {
    let game = gameObject();
    let homeTeamPoints = 0;
    let awayTeamPoints = 0;
  
    for (let teamKey in game) {
      let team = game[teamKey];
      let totalPoints = 0;
      for (let playerName in team.players) {
        totalPoints += team.players[playerName].points;
      }
      
      if (teamKey === "home") {
        homeTeamPoints = totalPoints;
      } else {
        awayTeamPoints = totalPoints;
      }
    }
  
    return homeTeamPoints > awayTeamPoints ? "Brooklyn Nets" : "Charlotte Hornets";
  }
  
  console.log(winningTeam()); // Should log the winning team

  function playerWithLongestName() {
    let game = gameObject();
    let longestName = "";
    let playerWithLongestName = "";
  
    for (let teamKey in game) {
      let team = game[teamKey];
      for (let playerName in team.players) {
        if (playerName.length > longestName.length) {
          longestName = playerName;
          playerWithLongestName = playerName;
        }
      }
    }
    return playerWithLongestName;
  }
  
  console.log(playerWithLongestName()); // Should log the player with the longest name

